...
const Footer = () => {
  ...
  const services = ['Business Analysis', 'CX Strategy', 'User Research', 'Process Optimization', 'Product Operations'];
          ...
          <div className="grid md:grid-cols-4 gap-8 mb-12">
            ...
            <div>
              ...
              <div className="space-y-2">
                {quickLinks.map(link => <button key={link.label} onClick={() => scrollToSection(link.href)} className="block text-gray-300 hover:text-white transition-colors text-left">
                    {link.label}
                  </button>)}
                <button onClick={() => scrollToSection('contact')} className="block text-gray-300 hover:text-white transition-colors text-left">
                  Contact
                </button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {services.map(service => <div key={service} className="text-gray-300 text-sm">
                    {service}
                  </div>)}
              </div>
            </div>
